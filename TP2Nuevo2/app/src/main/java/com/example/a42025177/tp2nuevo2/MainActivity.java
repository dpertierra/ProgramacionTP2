...
public class MainActivity extends AppCompatActivity {
    ...
    public int traerecord()
    {
        if (responder ==true) {
            Cursor ConjuntoRegistros;
            ConjuntoRegistros = basededatos.rawQuery("select Record from jugadores where Nombre=" + Nombre, null);
            if (ConjuntoRegistros.moveToFirst() == true) {

                do {int RecordSQL = ConjuntoRegistros.getInt(0);
                    record = RecordSQL;
                }
